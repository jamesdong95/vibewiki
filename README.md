...
**Latest verified preview:** [`v0.1.39-preview`](https://github.com/jamesdong95/vibewiki/releases/tag/v0.1.39-preview) · scan a repository locally or import a public GitHub URL, review file and graph changes after every rescan, configure product intent from the viewer, compare expected flows to implementation evidence, detect Astro/Nuxt filesystem routes, accept mixed root/nested package routes, browse common source/config formats, resolve workspace package imports, format grounded AI answers, show every graph edge in the summary, inspect source-linked facts, and observe a local runtime.
