...
- Bounded `POST /api/reviews/batch` atomically updates up to 100 review items
  after validating the complete batch.
- Unknowns queue now supports checkbox selection, Mark selected reviewed, and
  Reopen selected while preserving each item's existing note.
- Regression and browser smoke coverage for multi-select, bulk transitions,
  selection reset, and review counters.
